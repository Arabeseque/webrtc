...
export default {
  ...
  methods: {
    ...
    async call(item) {
      ...
      const params = { userId: getParams('userId'), targetUid: item.userId }
    ...
    async initCallerInfo(callerId, calleeId) {
      ...
      // 发送offer给被呼叫端
      const params = { targetUid: calleeId, userId: callerId, offer }
    ...
    async onRemoteOffer(fromUid, offer) {
      ...
      const params = { targetUid: fromUid, userId: getParams('userId'), answer }
      ...
      <el-col :span="6">
        <div style="width: 100%;height: 800px;">
          <ul v-for="(item, index) in roomUserList" :key="index">
            <el-tag size="mini" type="success" @click="getStats">
              {{ `用户${item.nickname}` }}
            </el-tag>
            <el-tag v-if="userInfo.userId === item.userId" type="danger" size="mini" @click="changeBitRate()">
              增加比特率
            </el-tag>
            <el-button v-if="userInfo.userId !== item.userId" size="mini" type="primary" @click="call(item)">
              通话
            </el-button>
            <el-button v-if="userInfo.userId === item.userId" size="mini" type="danger" @click="openVideoOrNot">
              切换
            </el-button>
          </ul>
        </div>
      ...
      <el-col :span="18">
        ...
        <el-row>
          <div style="display: flex;flex-direction: row;justify-content: flex-start;">
            <video id="localdemo01" autoplay controls muted @click="streamInfo('localdemo01')" />
            <video id="remoteVideo01" autoplay controls muted @click="streamInfo('remoteVideo01')" />
          </div>
        </el-row>
      </el-col>
    </el-row>
